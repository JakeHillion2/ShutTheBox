...
class OnlineScreen():
    ...
    def start_game(self):
        ...
        self.window.destroy()
        print('Starting game...')
        game = OnlineGame(self.joined_room, self.new_id, self.new_id_hash,self.opening_screen)


class OnlineGame():
    def __init__(self, game_id, uuid, uuid_hash,opening_screen):
    ...
    def offline_loop(self):
        ...
        for each in keys_to_process:
            event = self.events[each]
    ...
    def tell_server_dice_throw(self,x):
        self.send_server_event('DCT')

    def on_single_dice_change(self,x):
    ...
    def react_to_text_chat(self, data):
        self.chat_window.add_message(data['contents']['sender'],data['contents']['message'])
